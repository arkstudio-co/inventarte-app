...
-- ============================================
-- Ejecuta esto DESPUÉS de 00001_init.sql
-- ============================================
...
BEGIN
  ...
  IF existing_id IS NULL THEN
    ...
    VALUES (user_id, 'Admin Dibujarte', admin_email, 'admin');
...
BEGIN
  ...
  IF existing_id IS NULL THEN
    ...
    INSERT INTO public.profiles (id, full_name, email, role)
    VALUES (user_id, 'Operativo Dibujarte', op_email, 'operative');
...
-- 3. PROVEEDORES
INSERT INTO public.suppliers (name, contact, email, phone)
SELECT * FROM (VALUES
  ('Papelera Nacional', 'Carlos López', 'carlos@papeleranacional.com', '3001234567'),
  ('Art Supplies Colombia', 'María García', 'maria@artsupplies.co', '3107654321'),
  ('Distribuidora de Arte', 'Pedro Martínez', 'pedro@distribuidoraarte.com', '3209876543'),
  ('Insumos Gráficos SAS', 'Laura Fernández', 'laura@insumosgraficos.com', '3012345678')
) AS v(name, contact, email, phone)
...
BEGIN
  ...
  INSERT INTO public.products (sku, name, description, stock, min_stock, price, cost, supplier_id, created_by) VALUES
    ('ART-1A2-B3C', 'Block de Dibujo A3', 'Block profesional de dibujo, papel 120g/m², 50 hojas', 45, 10, 25000, 15000, s1_id, admin_id),
    ('LAP-4D5-E6F', 'Lápiz Grafito Profesional 2B', 'Lápiz de dibujo profesional, mina 2B, pack x12', 120, 20, 18000, 9000, s2_id, admin_id),
    ('ACU-7G8-H9I', 'Acuarelas 24 Colores', 'Estuche de acuarelas profesionales, 24 colores vibrantes', 30, 5, 65000, 38000, s2_id, admin_id),
    ('PIN-J1K-2L3', 'Pinceles Set x10', 'Set de pinceles profesionales, diferentes puntas y grosores', 25, 8, 42000, 25000, s3_id, admin_id),
    ('MAR-4M5-N6O', 'Marcadores Puntilla 12 colores', 'Marcadores de punta fina para lettering y detalles', 80, 15, 35000, 20000, s2_id, admin_id),
    ('OLE-7P8-Q9R', 'Óleo Profesional Set x12', 'Pintura al óleo profesional, colores básicos, tubos 37ml', 15, 3, 120000, 75000, s3_id, admin_id),
    ('CAR-S1T-2U3', 'Cartulina Opalina x10', 'Pliego de cartulina opalina, colores variados, pack x10', 200, 30, 15000, 8000, s1_id, admin_id),
    ('LIQ-V4W-5X6', 'Líquido Corrector 20ml', 'Corrector líquido blanco, secado rápido', 300, 50, 5000, 2500, s1_id, admin_id);

  RAISE NOTICE '✅ Productos de ejemplo insertados';
END $$;

-- 5. INFO DE EMPRESA (Landing Page)
INSERT INTO public.company_info (hero_title, hero_description, email, phone, about_text, work_history)
