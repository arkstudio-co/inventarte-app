...
export default function InventoryReportsPage() {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(13)
      doc.text('DIBUJARTE EDITORES', M, 42)

      doc.setFont('helvetica', 'normal')
      doc.setFontSize(7)
      doc.text('Nit: 123.456.789-0', M, 47)
      doc.text('Tel: (1) 234 5678', M, 51)
      doc.text('Email: info@dibujarte.com', M, 55)
      doc.text('Bogotá D.C., Colombia', M, 59)

      doc.setFont('helvetica', 'bold')
      doc.setFontSize(18)
      doc.setTextColor(...C.primary)
      doc.text('REPORTE DE INVENTARIO', pw / 2, 52, { align: 'center' })
      doc.setTextColor(0)

      doc.setFont('helvetica', 'normal')
      doc.setFontSize(9)
      doc.setTextColor(...C.inkSec)
      doc.text(`Generado el ${now}`, pw / 2, 60, { align: 'center' })
      doc.text(`Período: ${periodStr}`, pw / 2, 67, { align: 'center' })
      doc.text(`Total productos: ${products.length}`, pw / 2, 74, { align: 'center' })
